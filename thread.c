...
#define LIBC_CALL(func_call) \
    LIBC_CALL_CRITICAL++;    \
    func_call LIBC_CALL_CRITICAL--;
...
static int LIBC_CALL_CRITICAL = 0;
...
{
    printf("signal!\n NTHREADS:%d\n", nthreads);
    if (context.eip >= (int)_STARTMONITOR && context.eip >= (int)_ENDMONITOR)
    {
        printf("Inside thread library! returning.");
        return;
    }
    else if (LIBC_CALL_CRITICAL != 0)
    {
        printf("Inside LIBC/other library! returning.");
        return;
    }

    // LIBC_CALL(sigsetmask(context.oldmask);)
    Thread_pause();
    return;
}
...
int Thread_new(int func(void *), void *args, size_t nbytes, ...)
{
    ...
    LIBC_CALL(new_thread = malloc(1024 * 16 + nbytes + sizeof(thread_t) + 15);) // allocate bytes for stack + thread_t type -> +15 to be able to round up to already allocated memory
    ...
    if (nbytes > 0)
    {
        ...
        LIBC_CALL(memcpy(new_thread->sp, args, nbytes);) // copy args buffer into top of the stack
...
void cleanup_threads()
{
    while (!isEmpty(&free_queue))
    {
        ...
        {
            LIBC_CALL(free(finished_thread);)
        }
    }
}
